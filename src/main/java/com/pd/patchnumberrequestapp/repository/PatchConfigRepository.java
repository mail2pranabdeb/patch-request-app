...
public class PatchConfigRepository {
    ...
    public void updateConfig(PatchConfig config) {
        String sql = "UPDATE TOOL_PATCH_CONFIG SET open_book_rpp1 = ?, open_book_rpp2 = ?, open_book_rpp3 = ?, open_book_last_deployed_patch = ?, closed_book_rpp1 = ?, closed_book_rpp2 = ?, closed_book_rpp3 = ?, closed_book_last_deployed_patch = ? WHERE id = ?";
        jdbcTemplate.update(sql, 
                            config.getOpenBookRPP1(), config.getOpenBookRPP2(), config.getOpenBookRPP3(),
                            config.getOpenBookLastDeployedPatch(),
                            config.getClosedBookRPP1(), config.getClosedBookRPP2(), config.getClosedBookRPP3(),
                            config.getClosedBookLastDeployedPatch(),
                            config.getId());
    }
}
