...
public class PatchConfigRepository {
    ...
    private final RowMapper<PatchConfig> rowMapper = (rs, rowNum) -> {
        ...
        config.setOpenBookPatchFormat(rs.getString("open_book_patch_format"));
        ...
        config.setClosedBookPatchFormat(rs.getString("closed_book_patch_format"));
        ...
        config.setMaxMigPatchFormat(rs.getString("max_mig_patch_format"));
    ...
    public void updateConfig(PatchConfig config) {
        String sql = "UPDATE TOOL_PATCH_CONFIG SET " +
                     "open_book_rpp1 = ?, open_book_rpp2 = ?, open_book_rpp3 = ?, open_book_patch_format = ?, open_book_last_deployed_patch = ?, " +
                     "closed_book_rpp1 = ?, closed_book_rpp2 = ?, closed_book_rpp3 = ?, closed_book_patch_format = ?, closed_book_last_deployed_patch = ?, " +
                     "max_mig_rpp1 = ?, max_mig_rpp2 = ?, max_mig_rpp3 = ?, max_mig_patch_format = ?, max_mig_last_deployed_patch = ? WHERE id = ?";
        jdbcTemplate.update(sql, 
                            config.getOpenBookRPP1(), config.getOpenBookRPP2(), config.getOpenBookRPP3(), config.getOpenBookPatchFormat(), config.getOpenBookLastDeployedPatch(),
                            config.getClosedBookRPP1(), config.getClosedBookRPP2(), config.getClosedBookRPP3(), config.getClosedBookPatchFormat(), config.getClosedBookLastDeployedPatch(),
                            config.getMaxMigRPP1(), config.getMaxMigRPP2(), config.getMaxMigRPP3(), config.getMaxMigPatchFormat(), config.getMaxMigLastDeployedPatch(),
                            config.getId());
    }
}
